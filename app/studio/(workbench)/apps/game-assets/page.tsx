 ...
 */
export default async function GameAssetsAppRoute({ searchParams }: { searchParams: Promise<{ prompt?: string; projectId?: string }> }) {
  const params = await searchParams;
  const query = new URLSearchParams({ preset: "game-assets" });
  if (params.prompt) query.set("prompt", params.prompt);
  if (params.projectId) query.set("projectId", params.projectId);
  redirect(`/studio/create/image?${query.toString()}`);
}
