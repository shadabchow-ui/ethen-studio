...
function missingModelDecision(): PlannerDecision {
  return {
    decision: {
      intent: "fail",
      summary: "COMPUTER_USE_MODEL is not configured for OpenAI provider",
      confidence: "low",
      reason: "provider_not_configured",
    },
    providerMeta: {
      providerId: "openai",
      modelLabel: "OpenAI (model not configured)",
      mode: "llm",
    },
  };
}
...
function providerErrorDecision(message: string, isParseFailure = false): PlannerDecision {
  return {
    ...
    providerMeta: {
      providerId: "openai",
      modelLabel: "OpenAI (request failed)",
      mode: "llm",
    },
  };
}
...
function parseDecision(
  ...
  let raw: RawOpenAIDecision;
  try {
    raw = JSON.parse(jsonText) as RawOpenAIDecision;
  } catch {
...
async function callOpenAI(
  ...
  try {
    ...
    return { ok: false, error: `Network error calling OpenAI API: ${e instanceof Error ? e.message : "unknown"}` };
  }
  ...
  if (!response.ok) {
    ...
    return { ok: false, error: `OpenAI API request failed (${detail})` };
  }
  ...
  try {
    ...
    return { ok: false, error: "OpenAI API response was not valid JSON" };
  }

  const content = json.choices?.[0]?.message?.content;
  if (!content) {
    return { ok: false, error: "OpenAI API response did not contain message content" };
  }
...
export const openaiComputerProvider: ComputerUsePlannerProvider = {
  providerId: "openai",
  modelLabel: "OpenAI",
  mode: "llm",
  ...
  isConfigured(): boolean {
    ...
    if (configuredProvider === "openai" && !hasKey) return false;
    return hasKey || configuredProvider === "openai";
  },

  async planNextAction(input: PlannerDecisionInput): Promise<PlannerDecision> {
    const secretKey = getServerEnv("OPENAI_API_KEY");
    if (!secretKey) {
      return createProviderNotConfiguredDecision("openai");
    }
    ...
    const callResult = await callOpenAI(secretKey, model, systemPrompt, userPrompt);
    if (!callResult.ok) {
    ...
    const retryResult = await callOpenAI(secretKey, model, systemPrompt, repairPrompt);
    if (!retryResult.ok) {
      return providerErrorDecision(`OpenAI parse failed (${parsed.error}), retry also failed: ${retryResult.error}`, true);
    }

    const retryParsed = parseDecision(retryResult.text, allowedActionTypes);
    if (!retryParsed.ok) {
      return providerErrorDecision(`OpenAI response failed schema validation after retry: ${retryParsed.error}`, true);
    }
